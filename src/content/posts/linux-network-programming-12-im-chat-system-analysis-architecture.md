...
description: "Linux 网络编程课程第 12 课，分析 IM 聊天系统的基本功能与 C/S 架构，整理客户端和服务端的类职责、中介者模式及网络层抽象设计。"
image: ""
tags:
  - Linux
  - 网络编程
...
程序需要支持 TCP 和 UDP，因此不能把所有网络代码都堆在一个类中。课堂设计了一个抽象父类 `INet`，再由具体网络类继承并实现它。
...
因此课堂把 TCP 拆成 `TcpClient` 和 `TcpServer`，而 UDP 只设计一个 `Udp` 类。
